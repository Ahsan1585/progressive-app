...
-- Real platform-admin accounts (email + password) for /platform-admin —
-- replaces the old shared-secret (PLATFORM_ADMIN_KEY) gate. See
-- add_platform_admins.sql for the standalone migration; mirrored here for
-- a from-scratch install. totp_secret is unused until the 2FA fast-follow.
...
-- impersonation sessions. See add_platform_audit_logs.sql.

...
const getComplianceAnalysis = async (req, res) => {
  ...
  try {
    ...
    // Lazy sweep: state reference data older than 120 days ages out on its
    ...
    await pool.query("DELETE FROM compliance_state_logs WHERE service_date < CURRENT_DATE - INTERVAL '120 days'");

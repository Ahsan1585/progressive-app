...
export function MessageDock() {
  ...
  return (
    <>
      {/* Expanded chat windows — right-aligned row along the bottom edge */}
      ...
      {/* Directory panel */}
      {dirOpen && (
        <div className="fixed bottom-[4.75rem] right-4 z-50 flex h-[420px] w-[320px] flex-col overflow-hidden rounded-2xl border-2 border-slate-800 bg-white shadow-[0_20px_60px_-12px_rgba(0,0,0,0.55)]">
      ...
      {/* Minimized / overflow pills — stacked above the launcher */}
      {trayThreads.length > 0 && (
        <div className="fixed bottom-[4.75rem] right-4 z-40 flex flex-col items-end gap-1.5">
          {trayThreads.map((t) => (
            <button
              key={t.practitionerId}
              type="button"
      ...
      <button
        ...
        className="fixed bottom-4 right-4 z-50 flex size-14 items-center justify-center rounded-full bg-slate-900 text-white shadow-[0_10px_30px_-8px_rgba(0,0,0,0.5)] hover:bg-slate-800 cursor-pointer"
        aria-label="Messages"
      >

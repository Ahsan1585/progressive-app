...
export const BillingManager = () => {
  ...
  // Batch Review has its own search box and period picker — these no longer
  // have any UI control to change them, so they stay permanently blank
  // (equivalent to "no filter"), which fetchExpandedLogsFor below still
  // reads as query params.
  ...
  const fetchExpandedLogsFor = async (practitionerId) => {
    try {
  ...
  // Batch Review fetches its own pending-logs queue, so this no longer
  // needs to (nothing reads a top-level list here anymore) — its real
  // remaining job is resyncing already-expanded rows, which other shared
  // handlers (e.g. handleReconcile) still read from expandedLogs. Kept as
  // its own function (rather than inlined at call sites) since it's called
  // from many places below and by the 20s background poll.
  const fetchLogs = async () => {
    expandedRowsRef.current.forEach(id => fetchExpandedLogsFor(id));
  };
  ...
  // Fetches + renders every selected invoice's PDF pages into canvases, then
  // calls window.print() directly — no intermediate preview step. Note:
  // calling window.print() only after a chain of awaits (the fetch/render
  // work below) is the same pattern that broke the invoice viewer on iOS
  // Safari/PWA (see commit 5840a20) and, per user report, could make this
  // bulk-print flow silently navigate away instead of opening a print
  // dialog on some browsers. An in-app preview step was added specifically
  // to keep the actual print() call synchronous and fix that. Removed here
  // at explicit user request (skip the extra window, go straight to
  // print) — this reintroduces that risk on any browser sensitive to it.
  ...
  return (
    <div className="flex flex-col space-y-6">
      ...
      {/* TAB 1: PENDING BILLS */}
      {activeTab === 'pending' && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col relative left-1/2 -translate-x-1/2 w-[78.75vw] max-w-[1512px]">
          <BillingBatchReview
